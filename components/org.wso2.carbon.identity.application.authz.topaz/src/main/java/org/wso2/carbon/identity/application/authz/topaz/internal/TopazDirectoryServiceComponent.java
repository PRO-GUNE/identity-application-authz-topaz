...
import org.wso2.carbon.identity.application.authz.topaz.handler.impl.TopazDirectoryHandler;

@Component(
        name = "identity.application.authz.topaz.component",
        immediate = true
)
public class TopazDirectoryServiceComponent {
    ...
    protected void activate(ComponentContext ctxt) {
        try {
            ...
            ctxt.getBundleContext().registerService(DirectoryInterface.class, topazDirectoryHandler, null);
            if (log.isDebugEnabled()) {
                log.debug("Application Topaz directory handler bundle is activated");
            }
        } catch (Throwable throwable) {
            log.error("Error while starting topaz directory component", throwable);
        }
    }

    @Deactivate
    protected void deactivate(ComponentContext context) {
        log.debug("Application Topaz directory handler bundle is deactivated.");
    }
}
